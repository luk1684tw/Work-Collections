...
int proxy_func(int ser_port, int clifd, int rate , int loads);
int create_server(int port);
void rate_control(clock_t t , int rate , int buf_size);
...
int main (int argc, char **argv) {
    ...
    for (;;) {
        ...
        if ((childpid = fork()) == 0) {
            close(ctrlfd);
            proxy_func(FTP_PORT, connfd, rate , 0);
            ...
            if (FD_ISSET(serfd, &rset)) {
                ...
                if (status == FTP_PASV_CODE && ser_port == FTP_PORT) {
                    ...
                    if ((childpid = fork()) == 0) {
                        ...
                        proxy_func(data_port, connfd, rate , 1);
...
void rate_control(clock_t t , int rate , int buf_size) {
    ...
    long double target_time , time_delay, error_speed, errors;	
    target_time = (long double)((long double)size*1000000.0)/((long double)rate*1024.0);
	error_speed = (long double)size/(long double)t-(long double)rate;
	errors = (error_speed/(long double)rate)/
			 (long double)((long double)rate*((long double)rate+(long double)(error_speed/(long double)rate)));
	printf("----errors speed: %LF----\n", error_speed);
   	if (t <= target_time){   
       		time_delay = (long double)(target_time-t);
			printf("----delay: %LF----\n", time_delay+errors*1000000*(long double)(error_speed/(long double)rate));
			usleep(time_delay + errors*1000000*(long double)(error_speed/(long double)rate));
    }
}
